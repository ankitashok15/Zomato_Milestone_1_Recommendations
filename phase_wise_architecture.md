...
| Topic | Guidance |
|--------|-----------|
| **Platform** | **Streamlit Community Cloud** (or enterprise Streamlit hosting): connect the Git repository; main file **`streamlit_app.py`** (repo root); **`requirements.txt`** for packages; optional **`runtime.txt`** (e.g. `3.12.8`) to pin Python; **Secrets** e.g. `GROQ_API_KEY`, optional `GROQ_MODEL` (never commit `.env`). |
